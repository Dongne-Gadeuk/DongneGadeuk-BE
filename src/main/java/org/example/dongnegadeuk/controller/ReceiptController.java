...
import org.example.dongnegadeuk.service.ItemService;
...
public class ReceiptController {
    ...
    public BaseResponse<ReceiptDto> create(@AuthenticationPrincipal Users user, @RequestParam("image") MultipartFile image) throws IOException {
        ...
        ReceiptDto data = receiptService.process(user, image.getBytes());
        return BaseResponse.success("영수증 추출 완료", data);
    }

    @Operation(summary = "영수증 촬영", description = "영수증을 촬영해서 정보를 반환합니다.")
    @PostMapping("/receiveItem")
    public BaseResponse<ItemReceiveDto> receiveItem(@AuthenticationPrincipal Users user, @RequestBody ReceiptDto receiptDto) throws IOException {

        if (user == null) {
            throw new CustomException(JWT_MISSING);
        }

        ItemReceiveDto data = itemService.receiveItem(user, receiptDto);
        return BaseResponse.success("아이템 저장 완료!", data);
